...
export async function fetchHistoricalEvaluationRows(signalClient, marketClient, indexClient) {
  const [signals, returns, indexReturns] = await Promise.all([
  ...
  return signals.flatMap((signal) => {
    ...
    const indexReturn = indexReturnsByDate.get(signal.as_of_trade_date)
    if (!marketReturn || !indexReturn) return []
    return [{
      ...signal,
      ...
      next_csi1000_return: indexReturn.next_csi1000_return,
      next_csi2000_return: indexReturn.next_csi2000_return,
      next_sse_composite_return: indexReturn.next_sse_composite_return,
    }]
  })
}
