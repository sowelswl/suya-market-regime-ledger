...
export async function buildSiteData(root) {
  const [commitments, reveals, historicalEvaluation, revealedOutcomes, corrections] = await Promise.all([
    ...
    readOptionalJson(path.join(root, "evaluation/public/revealed-outcomes.json")),
  ...
  const outcomesByDate = new Map((revealedOutcomes?.records ?? []).map((outcome) => [outcome.as_of_trade_date, outcome]))
  ...
  return {
    ...
    privacy: {
      ...
      market_outcomes_after_the_fact: true,
    },
    ...
    records: commitments.slice(0, 20).map((commitment) => ({
        ...
        outcome: revealsByDate.has(commitment.as_of_trade_date)
          ? (outcomesByDate.get(commitment.as_of_trade_date) ?? null)
          : null,
