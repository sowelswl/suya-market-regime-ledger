...
function directionalReturn(level, realizedReturn) {
  return level >= 0 ? realizedReturn : -realizedReturn
}

function summarize(rows, returnField) {
  const returns = rows.map((row) => row[returnField])
  const directionalReturns = rows.map((row) => directionalReturn(row.signal_level, row[returnField]))
  ...
  return {
    ...
    mean_directional_return: round(mean(directionalReturns)),
...
function evaluationWindows(rows) {
  const endDate = rows.at(-1).as_of_trade_date
  return [
    { key: "all", label: "完整历史", startDate: rows[0].as_of_trade_date, rows },
    { key: "three_months", label: "近 3 个月", startDate: subtractCalendarMonths(endDate, 3) },
    { key: "one_month", label: "近 1 个月", startDate: subtractCalendarMonths(endDate, 1) },
  ].map((window) => {
    const windowRows = window.rows ?? rows.filter((row) => row.as_of_trade_date >= window.startDate)
    const benchmarks = {}
    for (const [key, benchmark] of Object.entries(BENCHMARKS)) {
      benchmarks[key] = {
        label: benchmark.label,
        role: benchmark.role,
        ...summarize(windowRows, benchmark.returnField),
      }
    }
    return {
      key: window.key,
...
export function buildHistoricalEvaluation(sourceRows, { generatedAt = new Date().toISOString() } = {}) {
  ...
  return {
    ...
    evaluation_windows: evaluationWindows(rows),
    source_snapshot: {
