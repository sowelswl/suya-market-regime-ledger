...
function normalizeRows(rows) {
  return rows.map((row) => {
    ...
    const csi500Return = Number(row.next_csi500_return)
    const hs300Return = Number(row.next_hs300_return)
    if (!Number.isFinite(csi500Return) || !Number.isFinite(hs300Return)) {
    ...
    return {
      ...
      next_csi500_return: csi500Return,
      next_hs300_return: hs300Return,
    }
...
export function buildHistoricalEvaluation(sourceRows, { generatedAt = new Date().toISOString() } = {}) {
  ...
  return {
    ...
    source_snapshot: {
      ...
      market_return_source: "aistk.public.micro_timing_final_tail_hold_dates:csi500_return,hs300_return",
      algorithm: "sha256",
      sha256: snapshot,
